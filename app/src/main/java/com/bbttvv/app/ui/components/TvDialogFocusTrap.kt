...
@Composable
internal fun rememberTvDialogFocusTrap(
    returnFocusKey: String? = null,
    returnFocusFallbackKeys: List<String> = emptyList(),
): FocusRequester {
    val ownerView = LocalView.current
    val focusRequester = remember { FocusRequester() }
    val viewFocusReturn = remember(ownerView) { DialogFocusReturn() }
    val keyFocusReturn = LocalTvFocusReturn.current

    DisposableEffect(ownerView, keyFocusReturn, returnFocusKey, returnFocusFallbackKeys) {
        val ownerRoot = ownerView.rootView ?: ownerView
        viewFocusReturn.capture(ownerRoot.findFocus())
        if (returnFocusKey != null) {
            keyFocusReturn?.capture(
                key = returnFocusKey,
                fallbackKeys = returnFocusFallbackKeys,
            )
        }
        ...
        onDispose {
            ...
            val restoredByKey = if (returnFocusKey != null) {
                keyFocusReturn?.restore() == true
            } else {
                false
            }
            if (restoredByKey) {
                viewFocusReturn.clear()
            } else {
                viewFocusReturn.restoreAndClear(fallback = ownerView)
            }
            ownerView.post {
                ownerView.post {
