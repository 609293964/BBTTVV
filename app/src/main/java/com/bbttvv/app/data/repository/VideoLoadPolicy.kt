...
internal fun shouldSkipPlayUrlCache(
    ...
    return audioLang != null || isAutoHighestQuality
}

internal fun buildDashAttemptQualities(targetQn: Int): List<Int> {
    if (targetQn <= 80) return listOf(targetQn)

    val premiumFallbacks = listOf(127, 126, 125, 120, 116, 112)
        .filter { quality -> comparePlaybackQuality(quality, targetQn) <= 0 }
...
internal fun shouldAcceptAppApiResultForTargetQuality(
    ...
    if (requestKind == PlayUrlRequestKind.AUTO_HIGHEST) {
        val playableQuality = highestPlaybackQuality(dashVideoIds + listOf(returnedQuality))
            ?: return false
        val advertisedHighest = highestPlaybackQualityNotAbove(
            qualities = acceptQualities + dashVideoIds + listOf(returnedQuality),
            targetQn = targetQn
        ) ?: playableQuality
        return comparePlaybackQuality(playableQuality, advertisedHighest) >= 0
    }
...
private val playbackQualityOrder = listOf(6, 16, 32, 64, 74, 80, 100, 112, 116, 120, 125, 126, 127, 129)

private fun playbackQualityRank(quality: Int): Int {
    val index = playbackQualityOrder.indexOf(quality)
    return if (index >= 0) index else playbackQualityOrder.size + quality
}

internal fun comparePlaybackQuality(left: Int, right: Int): Int {
    return playbackQualityRank(left).compareTo(playbackQualityRank(right))
}

private fun highestPlaybackQuality(qualities: List<Int>): Int? {
    return qualities
        .filter { it > 0 }
        .distinct()
        .maxWithOrNull { left, right -> comparePlaybackQuality(left, right) }
}

private fun highestPlaybackQualityNotAbove(
    qualities: List<Int>,
    targetQn: Int
): Int? {
    return qualities
        .filter { quality -> quality > 0 && comparePlaybackQuality(quality, targetQn) <= 0 }
        .distinct()
        .maxWithOrNull { left, right -> comparePlaybackQuality(left, right) }
}
