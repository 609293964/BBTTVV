...
object ActionRepository : UserActionRepository {
    ...
    suspend fun getUserFollowGroupIds(mid: Long): Result<Set<Long>> {
        return withContext(Dispatchers.IO) {
            ...
            val fallbackMessage = if (lastMessage.isNotBlank()) lastMessage else "获取分组信息失败: $lastCode"
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                while (page <= FOLLOW_GROUP_TAG_MEMBERS_MAX_PAGES) {
                    ...
                    if (targetSet == null) result.addAll(mids)
                    else mids.filterTo(result) { targetSet.contains(it) }
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                midChunks.forEachIndexed { index, mids ->
                    ...
                    val resetResult = addUsersToRelationTagsWithRetry(fids, "0", csrf)
                    if (resetResult.isFailure) {
                        return@withContext Result.failure(resetResult.exceptionOrNull() ?: Exception("分组设置失败"))
                    }
                    if (normalizedSelection.isNotEmpty()) {
                        val applyResult = addUsersToRelationTagsWithRetry(fids, selectedTagIdsJoined, csrf)
    ...
    override suspend fun checkFavoriteStatus(aid: Long): Boolean {
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (response.code == 0) response.data?.favoured ?: false else false
            } catch (e: CancellationException) {
                throw e
            } catch (e: Exception) {
    ...
    override suspend fun checkLikeStatus(aid: Long): Boolean {
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (response.code == 0) response.data == 1 else false
            } catch (e: CancellationException) {
                throw e
            } catch (e: Exception) {
    ...
    suspend fun toggleWatchLater(aid: Long, add: Boolean): Result<Boolean> {
        return withContext(Dispatchers.IO) {
            try {
                val csrf = TokenManager.csrfCache ?: ""
                if (csrf.isEmpty()) return@withContext Result.failure(Exception("请先登录"))
                val response = if (add) api.addToWatchLater(aid = aid, csrf = csrf)
                else api.deleteFromWatchLater(aid = aid, csrf = csrf)
                when (response.code) {
