...
object LiveRepository {
    ...
    private const val LIVE_AREAS_CACHE_TTL_MS = 12 * 60 * 60 * 1000L
    ...
    @Volatile
    private var liveAreasCache: LiveAreasCache? = null
    ...
    suspend fun getLiveRooms(page: Int = 1, parentAreaId: Int = 0): Result<List<LiveRoom>> = withContext(Dispatchers.IO) {
        safeApiCall(tag = TAG, errorMessage = { "getLiveRooms failed: parentAreaId=$parentAreaId, page=$page" }) {
            val resp = api.getLiveList(parentAreaId = parentAreaId, page = page)
            val list = resp.data?.getAllRooms() ?: emptyList()
            list.firstOrNull()?.let {
                Logger.d(TAG, "🟢 Popular Live: roomid=${it.roomid}, title=${it.title}, online=${it.online}")
            }
            Logger.d(TAG, "🔴 getLiveRooms parentAreaId=$parentAreaId, page=$page, count=${list.size}")
            list
        }
    }

    suspend fun getLiveAreas(forceRefresh: Boolean = false): Result<List<LiveAreaParent>> = withContext(Dispatchers.IO) {
        safeApiCall(tag = TAG, errorMessage = { "getLiveAreas failed: forceRefresh=$forceRefresh" }) {
            val now = System.currentTimeMillis()
            val cached = liveAreasCache
            if (!forceRefresh && cached != null && now - cached.fetchedAtMs < LIVE_AREAS_CACHE_TTL_MS) {
                return@safeApiCall cached.items
            }
            val resp = api.getLiveAreaList()
            if (resp.code != 0) throw Exception(resp.message.ifBlank { "直播分类加载失败(${resp.code})" })
            val areas = resp.data?.filter { it.id > 0 && it.name.isNotBlank() } ?: emptyList()
            liveAreasCache = LiveAreasCache(fetchedAtMs = now, items = areas)
            Logger.d(TAG, "🔴 getLiveAreas count=${areas.size}")
            areas
        }
    }

    suspend fun warmupLiveMetadata(): Int = getLiveAreas(forceRefresh = false).getOrNull()?.size ?: 0

    suspend fun getLiveRoomDetail(roomId: Long): Result<LiveRoomDetailData> = withContext(Dispatchers.IO) {
        safeApiCall(tag = TAG, errorMessage = { "getLiveRoomDetail failed: roomId=$roomId" }) {
            val realRoomId = resolveRealRoomId(roomId)
            val resp = api.getLiveRoomDetail(roomId = realRoomId)
            if (resp.code == 0 && resp.data != null) resp.data
            else throw Exception(resp.message.ifBlank { "直播间详情加载失败(${resp.code})" })
        }
    }

    suspend fun getFollowedLive(page: Int = 1): Result<List<LiveRoom>> = withContext(Dispatchers.IO) {
        safeApiCall(tag = TAG, errorMessage = { "getFollowedLive failed: page=$page" }) {
            val resp = api.getFollowedLive(page = page)
            val followedRooms = resp.data?.list?.filter { it.liveStatus == 1 } ?: emptyList()
            followedRooms.map { room ->
                ...
                try {
                    ...
                    android.util.Log.w(TAG, "Failed to get room info for ${room.roomid}: ${error.message}")
                    liveRoom
                }
            }
        }
    ...
    suspend fun getLivePlayUrl(roomId: Long): Result<String> = withContext(Dispatchers.IO) {
        safeApiCall(tag = TAG, errorMessage = { "getLivePlayUrl failed: roomId=$roomId" }) {
            val realRoomId = resolveRealRoomId(roomId)
            Logger.d(TAG, "🔴 Fetching live URL for roomId=$roomId(real=$realRoomId)")
            val resp = api.getLivePlayUrl(roomId = realRoomId)
            Logger.d(TAG, "🔴 Live API response: code=${resp.code}, msg=${resp.message}")
            ...
            if (playurlInfo != null) {
                ...
                if (codec != null && urlInfo != null) {
                    return@safeApiCall preferHttpsUrl(urlInfo.host + codec.baseUrl + urlInfo.extra)
                }
            }

            val url = resp.data?.durl?.firstOrNull()?.url
            if (url != null) return@safeApiCall preferHttpsUrl(url)
            throw Exception("无法获取直播流")
        }
    }
    ...
    suspend fun getLivePlayUrlWithQuality(roomId: Long, qn: Int = 10000): Result<LivePlayUrlData> = withContext(Dispatchers.IO) {
        try {
            val realRoomId = resolveRealRoomId(roomId)
            Logger.d("LiveRepo", "🔴 Fetching live URL with quality for roomId=$roomId(real=$realRoomId), qn=$qn")
            ...
            val legacyResp = try {
                ...
                android.util.Log.w("LiveRepo", "Legacy API failed: ${error.message}")
                null
            }
            ...
            val resp = try {
                ...
                android.util.Log.w("LiveRepo", "xlive API failed: ${error.message}")
                null
            }
            ...
            Logger.e("LiveRepo", "getLivePlayUrlWithQuality failed: roomId=$roomId, qn=$qn", error)
            Result.failure(error)
        }
    }

    suspend fun sendDanmaku(roomId: Long, msg: String): Result<Boolean> = withContext(Dispatchers.IO) {
        safeApiCall(tag = TAG, errorMessage = { "sendDanmaku failed: roomId=$roomId" }) {
            val realRoomId = resolveRealRoomId(roomId)
            val csrf = com.bbttvv.app.core.store.TokenManager.csrfCache ?: ""
            if (csrf.isEmpty()) throw Exception("请先登录")
            ...
            if (resp.code == 0) true else throw Exception(resp.message.ifBlank { "发送失败" })
        }
    }

    suspend fun clickLike(roomId: Long, uid: Long, anchorId: Long): Result<Boolean> = withContext(Dispatchers.IO) {
        safeApiCall(tag = TAG, errorMessage = { "clickLike failed: roomId=$roomId" }) {
            val realRoomId = resolveRealRoomId(roomId)
            val csrf = com.bbttvv.app.core.store.TokenManager.csrfCache ?: ""
            ...
            if (resp.code == 0) true else throw Exception(resp.message)
        }
    }

    suspend fun getEmoticons(roomId: Long): Result<Map<String, String>> = withContext(Dispatchers.IO) {
        safeApiCall(tag = TAG, errorMessage = { "getEmoticons failed: roomId=$roomId" }) {
            val realRoomId = resolveRealRoomId(roomId)
            ...
            if (resp.code == 0 && resp.data?.data != null) {
                ...
                Logger.d(TAG, " Fetched ${emojiMap.size} emoticons for room $roomId(real=$realRoomId)")
                emojiMap
            } else {
