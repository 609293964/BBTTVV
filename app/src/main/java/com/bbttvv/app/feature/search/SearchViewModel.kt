...
class SearchViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun search() {
        ...
        val keyword = if (typedKeyword.isNotBlank()) {
            ...
            state.defaultSearchKeyword.trim()
        } else {
            ""
        }
        if (keyword.isBlank()) {
    ...
    private fun loadDefaultSearchHint() {
        viewModelScope.launch {
            SearchRepository.getDefaultSearchHint().onSuccess { hint ->
                if (hint.displayName.isNotBlank() && hint.keyword.isNotBlank()) {
                    _uiState.update {
                        it.copy(
                            defaultSearchHint = hint.displayName,
                            defaultSearchKeyword = hint.keyword,
