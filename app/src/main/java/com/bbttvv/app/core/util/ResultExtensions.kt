...
/**
 * `runCatching` 会把 CancellationException 当普通失败捕获；挂起代码统一使用这个
 * helper，保证协程取消仍然向上传播。
 */
...
suspend inline fun <T> safeApiCallOrDefault(
    ...
    return safeApiCall(tag = tag, errorMessage = errorMessage) { block() }.getOrDefault(default)
}
...
suspend inline fun <T> safeApiCallOrNull(
    ...
    return safeApiCall(tag = tag, errorMessage = errorMessage) { block() }.getOrNull()
}
...
inline fun <T> Result<T>.onFailureLog(
    tag: String,
    message: (Throwable) -> String = { it.message ?: "Unknown error" }
): Result<T> {
